...
def merge_daily_files(wct_dir, output_dir, filter_dir, movstack_dir, component_dir, pair_dir, logger):
            ...
            if len(dates) == 1:
                ...
                if 'component' in data:
                    try:
                        ...
                        component = component_dir
                # Read stack config saved by compute_wct
                if 'curr_window_days' in data:
                    curr_window_days = int(data['curr_window_days'])
                if 'ref_window_days' in data:
                    ref_window_days  = int(data['ref_window_days'])
                if 'step_days' in data:
                    step_days        = int(data['step_days'])
                if 'ref_lag_days' in data:
                    ref_lag_days     = int(data['ref_lag_days'])
    ...
    if curr_window_days is not None:
        attrs['curr_window_days'] = curr_window_days
        attrs['ref_window_days']  = ref_window_days
        attrs['step_days']        = step_days
        attrs['ref_lag_days']     = ref_lag_days
    else:
        logger.warning(
            "Stack config missing from .npz files in %s. "
            "DvvIntegrator will not derive parameters automatically. "
            "Re-run compute_wct to regenerate the .npz files." % full_path
        )
    dvv = xr.DataArray(
        dvv_df.values,
        dims=['times', 'freq'],
    ...
    try:
        ...
        if any(v is None for v in [curr_window_days, step_days, ref_window_days, ref_lag_days]):
            logger.warning(
                f"Stack config attributes missing from .npz files in {full_path}. "
                f"DvvIntegrator will not be able to derive integration parameters automatically. "
                f"Re-run compute_wct to regenerate the .npz files."
            )
...
def main():
    ...
    parser.add_argument('--output_dir', default='WCT_MERGED_dyno_user7', help='Directory to save merged results')
