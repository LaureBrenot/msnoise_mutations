...
@click.option('-a', '--all', is_flag=True, help='Reset all jobs')
@click.option('-r', '--rule', help='Reset job that match this SQL rule')
@click.option('--reset-i/--no-reset-i', default=True, 
              help='Reset jobs with "I" (In progress) flag (default: True)')
@click.option('--reset-e/--no-reset-e', default=True, 
              help='Reset jobs with "E" (Error) flag (default: True)')
def reset(jobtype, all, rule, reset_i, reset_e):
    """Resets the jobs to "T"odo. JOBTYPE is the acronym of the job type.
    
    By default resets both "I"n progress and "E"rror jobs. Use --no-reset-i or 
    --no-reset-e to skip resetting jobs with specific flags.
    
    --all resets all jobs, whatever the flag value.
    
    Standard Job Types are CC, STACK, MWCS, DTT, and WCT, but plugins can 
    define their own."""
    from ..api import connect, reset_jobs, read_db_inifile
    from sqlalchemy import text
    ...
    reset_jobs(session, jobtype, all, rule, reset_i=reset_i, reset_e=reset_e)
    
    # Report what was reset
    if rule:
        logging.info(f"Reset jobs of type {jobtype} matching rule: {rule}")
    elif all:
        logging.info(f"Reset ALL jobs of type {jobtype}")
    else:
        flags = []
        if reset_i:
            flags.append('"I"n progress')
        if reset_e:
            flags.append('"E"rror')
        logging.info(f"Reset {' and '.join(flags)} jobs of type {jobtype}")
...
def dvv_merge_wct(ctx, wct_dir, output_dir):
    ...
    loglevel = ctx.obj['MSNOISE_verbosity']
    threads = ctx.obj['MSNOISE_threads']
    main(wct_dir=wct_dir, output_dir=output_dir, loglevel=loglevel, num_processes=threads)
