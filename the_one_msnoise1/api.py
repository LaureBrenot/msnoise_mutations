...
def reset_jobs(session, jobtype, alljobs=False, rule=None, reset_i=True, reset_e=True):
    """
    Sets the flag of selected Jobs to "T"odo.
    ...
    :param jobtype: CrossCorrelation (CC) or dt/t (DTT) Job?
    :type alljobs: bool
    :param alljobs: If True, resets all jobs regardless of flag. If False (default), 
...
def xr_get_ref2(station1, station2, components, filterid, taxis):
    path = os.path.join("STACKS2", "%02i" % filterid,
                        "REF", "%s" % components)
    fn = "%s_%s.nc" % (station1, station2)
...
def xr_save_wct(station1, station2, components, filterid, mov_stack, taxis, dvv_df, err_df, coh_df, output_dir="WCT"):
    """
    ...
    fn = os.path.join(output_dir, f"{filterid:02d}", f"{mov_stack[0]}_{mov_stack[1]}",
