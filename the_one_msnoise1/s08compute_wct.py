        ...
        for component in components_to_compute:
            ...
            for mov_stack in mov_stacks:
                try:
                    ...
                    dynamic = True
                    if dynamic:
                        ref_start = date + pd.Timedelta(days=int(-30))#params.ref_begin))  # e.g. -30 days before current date
                        ref_end = date + pd.Timedelta(days=int(-1))#params.ref_end))   # e.g. -1 day before current date
                        start_date = pd.Timestamp(params.startdate)
                        print(ref_start,ref_end, date)
                        
                        if ref_start < start_date:
                            logger.debug(f"Reference period {ref_start} would be before data start {start_date}, skipping")
                            continue
