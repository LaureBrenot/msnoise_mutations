...
#                    dynamic = True
#                    if dynamic:
#                        ref_start = date + pd.Timedelta(days=int(-19))#params.ref_begin))  # e.g. -30 days before current date
#                        ref_end = date + pd.Timedelta(days=int(-10))#params.ref_end))   # e.g. -1 day before current date
#                        start_date = pd.Timestamp(params.startdate)
#                        print(ref_start,ref_end, date)
#                        logger.debug(f"Reference period {ref_start} {ref_end}, {date} ")
#                        
#                        if ref_start < start_date:
#                            logger.info(f"Reference period {ref_start} would be before data start {start_date}, skipping")
#                            continue
#        
#                        try:
#                            ref_data = xr_get_ccf(station1, station2, component, filterid, mov_stack, taxis)
#                            ref_data = ref_data[(ref_data.index >= ref_start) & (ref_data.index <= ref_end)]
#                            
#                            # First get it into DataFrame format if it's not already
#                            if isinstance(ref_data, xr.Dataset):
#                                ref_data = ref_data.CCF.to_dataframe()
#                            elif isinstance(ref_data, xr.DataArray):
#                                ref_data = ref_data.to_dataframe()
#                            
#                            if len(ref_data) == 0:
#                                logging.warning(f"No reference data found between {ref_start} and {ref_end}")
#                                continue
#                            # Calculate mean over time
#                            ref = ref_data.mean(axis=0) 
#                            ref = pd.DataFrame(ref, columns=['CCF'])
#                            
#                            # Convert to numpy array 
#                            if wct_norm:
#                                ori_waveform = (ref/ref.max()).values.flatten()  
#                            else:
#                                ori_waveform = ref.values.flatten()  
##                            print("ref_data shape:", ref_data.shape if hasattr(ref_data, 'shape') else "no shape")
##                            print("ref_data type:", type(ref_data))
##                            print("ref max value:", ref.max())
##                            print("After normalization (dynamic):", ori_waveform.max())
#                            #ori_waveform = ori_waveform.values.astype(np.float64)
#                            
#                        except FileNotFoundError:
#                          logger.error(f"Could not load reference data for {date}")
#                          continue
#                            
#                        ##end modif
#                    
...
#                    continue
            for mov_stack in mov_stacks:
                try:
                    # Parse stack size in days from mov_stack[0], e.g. ('10d', '10d') ? 10
                    stack_size = int(''.join(filter(str.isdigit, str(mov_stack[0]).split('d')[0])))
            
                    # Load all stacked CCF data once for this mov_stack
                    all_data = xr_get_ccf(station1, station2, component, filterid, mov_stack, taxis)
            
                    # Normalise current date and extract the single stack row for this job
                    ...
                    # -- Dynamic reference: most recent non-overlapping stack --
                    #
                    # The index in all_data is the END date of each stack window.
                    # Current window covers: (date - stack_size + 1) … date
                    # So any stack whose index <= (date - stack_size) cannot overlap.
                    #
                    # This works across gaps: if data jumps from -60 to -10 to 0 to 10:
                    #   date= 0  ? ref = -10  (previous stack, adjacent)
                    #   date=-10 ? ref = -60  (jumps over gap, still no overlap)
                    #   date=10  ? ref =   0  (previous stack becomes ref)
