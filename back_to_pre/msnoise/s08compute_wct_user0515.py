...
def process_wct_job_adaptive(pair, day, params, taxis, filters, distances_cache=None): 
    ...
    if distances_cache:
        ...
        if distance_km:
            logger.debug(f"Using cached distance between {station1} and {station2}: {distance_km:.2f} km")
        else:
            logger.warning(f"No cached distance found for {station1}-{station2}, will use minimum coda time")
    else:
        logger.warning("No distances cache provided, will use minimum coda time for all pairs")
    ...
    velocity = params.coda_velocity
    safety_factor = params.coda_safety_factor
    min_coda_time = params.dtt_minlag  
    ...
    for f in filters:
        ...
        for component in components_to_compute:
            for mov_stack in mov_stacks:
                try:
                    if ref_type == 'static':
                        # Static: use the pre-computed MSNoise reference waveform
                        try:
                            ref = xr_get_ref(station1, station2, component, filterid, taxis, ignore_network=True)
                            ref_values = ref.CCF.values
                        except FileNotFoundError as fullpath:
                            logger.error(f"FILE DOES NOT EXIST: {fullpath}, skipping")
                            continue
                        except Exception as e:
                            logger.error(f"Error getting static reference waveform: {str(e)}")
                            continue
            
                        if not len(ref_values):
                            logger.warning(f"Empty static reference for {station1}:{station2} {component} f{filterid}")
                            continue
            
                        ori_waveform = ref_values / ref_values.max() if wct_norm else ref_values
                        logger.debug(f"Using static reference for {station1}:{station2} {component} f{filterid}")
                        all_data = xr_get_ccf(station1, station2, component, filterid, mov_stack, taxis)
                    ...
                    elif ref_type == 'moving':
#                        if params.wct_ref_start_days >= params.wct_ref_end_days:
#                            logger.error(
#                                f"wct_ref_start_days ({params.wct_ref_start_days}) must be less than "
#                                f"wct_ref_end_days ({params.wct_ref_end_days})"
#                            )
#                            return

                        # Parse current and step sizes from mov_stack
                        curr_window_days = int(''.join(filter(str.isdigit, str(mov_stack[0]).split('d')[0])))
                        step_days        = int(''.join(filter(str.isdigit, str(mov_stack[1]).split('d')[0])))
                        
                        # ref_window = same as curr (moving ref uses same mov_stack, just shifted back)
                        ref_window_days  = curr_window_days
                        ref_lag_days     = curr_window_days  # gap is always one current window
                        
                        stack_size = curr_window_days  # keep existing variable name for safe_boundary calc
                        all_data = xr_get_ccf(station1, station2, component, filterid, mov_stack, taxis)
                        
#                        stack_size = int(''.join(filter(str.isdigit, str(mov_stack[0]).split('d')[0])))
#                        all_data = xr_get_ccf(station1, station2, component, filterid, mov_stack, taxis)
...
#                        ref_end_days = params.wct_ref_end_days      # e.g. -10 (must be >= stack_size)
#                        ref_start_days = params.wct_ref_start_days  
#    
#                        if abs(ref_end_days) < stack_size:
#                            logger.error(
#                                f"wct_ref_end_days ({ref_end_days}) is closer than stack_size ({stack_size}d) "
#                                f"- reference would overlap current window, skipping"
#                            )
#                            continue
#                    
#                        ref_window_end = date + pd.Timedelta(days=ref_end_days)      # e.g. date - 10d
#                        ref_window_start = date + pd.Timedelta(days=ref_start_days)  # e.g. date - 40d
#                    
#                        # Check ref window doesn't go before data start
#                        if ref_window_start < pd.Timestamp(params.startdate):
#                            logger.info(
#                                f"Reference window start {ref_window_start.date()} is before data start "
#                                f"{params.startdate}, clamping to data start"
#                            )
#                            ref_window_start = pd.Timestamp(params.startdate)
#                    
#                        # Select all stacks within the reference window and average them
#                        ref_candidates = all_data[
#                            (all_data.index >= ref_window_start) & 
#                            (all_data.index <= ref_window_end)
#                        ]
#                    
#                        if ref_candidates.empty:
#                            logger.warning(
#                                f"No reference stacks found between {ref_window_start.date()} and "
#                                f"{ref_window_end.date()} for {station1}:{station2} {component} "
#                                f"f{filterid} m{mov_stack} at {date.date()}, skipping"
#                            )
#                            continue
#                    
#                        # Average all stacks in the reference window
#                        ref_values = ref_candidates.values.mean(axis=0)
#                        ori_waveform = ref_values / ref_values.max() if wct_norm else ref_values
#                    
#                        logger.debug(
#                            f"Moving ref: averaged {len(ref_candidates)} stacks from "
#                            f"{ref_window_start.date()} to {ref_window_end.date()} "
#                            f"(stack_size={stack_size}d, no-overlap guaranteed by ref_end_days={ref_end_days})"
#                        )
#                    else:
#                        logger.error(f"Unknown wct_ref_type '{ref_type}', expected 'static' or 'moving'")
#                        continue
#                        safe_boundary = date - pd.Timedelta(days=stack_size)
#                        ref_candidates = all_data[all_data.index <= safe_boundary]
#    
#                        if ref_candidates.empty:
#                            logger.warning(
#                                f"No reference stack available at or before {safe_boundary.date()} "
#                                f"for {station1}:{station2} {component} f{filterid} m{mov_stack} "
#                                f"@ {date.date()}, skipping"
#                            )
#                            continue
#    
#                        ref_row = ref_candidates.iloc[[-1]]  # most recent non-overlapping stack
#    
#                        if ref_row.index[0] < pd.Timestamp(params.startdate):
#                            logger.info(
#                                f"Best available reference {ref_row.index[0].date()} is before "
#                                f"data start {params.startdate}, skipping"
#                            )
#                            continue
#    
...
#                        ori_waveform = ref_values / ref_values.max() if wct_norm else ref_values
                        ...
                        # Replace the safe_boundary / iloc[[-1]] block with:
                        ref_end   = date - pd.Timedelta(days=stack_size)        # no overlap
                        ref_start = ref_end - pd.Timedelta(days=10)             # 30-day average window
                        
                        ref_candidates = all_data[
                            (all_data.index >= ref_start) & (all_data.index <= ref_end)
                        ]
                        
                        if ref_candidates.empty:
                            logger.warning(f"No reference stacks found between {ref_start.date()} and "
                                           f"{ref_end.date()}, skipping")
                            continue
                        
                        ref_values = ref_candidates.values.mean(axis=0)         # average like the old code
                        ori_waveform = ref_values / np.abs(ref_values).max() if wct_norm else ref_values

                        logger.debug(
                            f"Dynamic ref: {ref_row.index[0].date()} "
                            f"(safe_boundary={safe_boundary.date()}, current end={date.date()}, "
                            f"stack_size={stack_size}d, "
                            f"separation={(date - ref_row.index[0]).days}d)"
                        )
                    else:
                        logger.error(f"Unknown wct_ref_type '{ref_type}', expected 'static' or 'moving'")
                        continue             
                    ...
                    save_day_wct_results_adaptive(
                        ...
                        curr_window_days=curr_window_days, ref_window_days=ref_window_days, step_days=step_days, ref_lag_days=ref_lag_days
                    )
...
def save_day_wct_results_adaptive(station1, station2, date, component, filterid, mov_stack, 
                                 taxis, dvv, err, coh, freqs, inx, coda_diagnostics, curr_window_days, ref_window_days, step_days, ref_lag_days):
    """
...
def main(loglevel="INFO", batch_size=5):
    ...
    try:
        ...
        params.coda_safety_factor = 1.2
        params.dtt_minlag = 6.0 
        params.ref_type = 'moving'
        params.wct_ref_end_days   = -10   # reference window ends 10 days before current date
        params.wct_ref_start_days = -21   # reference window starts 40 days before current date
