...
def compute_wct_dvv_adaptive(freqs, tvec, WXamp, Wcoh, delta_t, distance_km=None,
    ...
    -------
    tuple
        dvv values (percentage), errors (percentage), weighting function,
        and coda diagnostics.  Frequencies that could not be estimated
        return NaN, never 0.0.
    """
    import warnings
    ...
    if distance_km is not None or ref_waveform is not None:
        ...
        logger.info(f"Adaptive coda window: lag_min = {lag_min:.2f}s, "
                    f"constraint = {coda_diagnostics.get('constraint', 'unknown')}")
    else:
    ...
    # FIX 3: NaN, not zeros.  A skipped frequency must not look like a
    # measurement of "no velocity change" to the integrator downstream.
    dvv = np.full(len(inx[0]), np.nan)
    err = np.full(len(inx[0]), np.nan)

    # Weighting function based on WXamp
    #weight_func = np.log(np.abs(WXamp)) / np.log(np.abs(WXamp)).max()
    with np.errstate(divide='ignore', invalid='ignore'):
        L = np.log(np.abs(WXamp))
    L = np.where(np.isfinite(L), L, np.nan)
    Lmin, Lmax = np.nanmin(L), np.nanmax(L)
    # FIX 2: mask NaNs explicitly.  xwt() leaves Wcoh and delta_t as NaN
    # wherever the wavelet had no power, and NaN comparisons return False,
    # so those samples would otherwise pass through with full weight.
    # Also uses abs(delta_t): delta_t is signed and the clip in xwt() is
    # commented out, so large negative delays were never rejected.
    zero_idx = np.where(
        ~np.isfinite(Wcoh) |
        ~np.isfinite(delta_t) |
        (Wcoh < mincoh) |
        (np.abs(delta_t) > maxdt)
    )
    if not np.isfinite(Lmin) or not np.isfinite(Lmax) or Lmax <= Lmin:
        logger.warning("Degenerate WXamp dynamic range, falling back to uniform weights")
        wf = np.ones(L.shape, dtype=float)
    else:
        wf = (L - Lmin) / (Lmax - Lmin)
    ...
    for ii, ifreq in enumerate(inx[0]):
        ...
        tindex = np.where(((tvec >= -lag_max) & (tvec <= -lag_min)) |
                          ((tvec >= lag_min) & (tvec <= lag_max)))[0]
        ...
        if not np.any(np.isfinite(delta_t[ifreq])):
            continue

        # FIX 1: no nan_to_num.  It replaced undefined delays with genuine
        # zero-delay measurements and mutated delta_t in place.  Drop the
        # bad samples instead of inventing them.
        seg   = delta_t[ifreq][tindex]
        w_seg = wf[ifreq][tindex]
        good  = np.isfinite(seg) & np.isfinite(w_seg) & (w_seg > 0)

        if good.sum() / len(tindex) < min_nonzero:
            continue                      # dvv/err stay NaN

        with warnings.catch_warnings(record=True) as w_catcher:
            warnings.simplefilter("always", OptimizeWarning)
            m, em = linear_regression(tvec[tindex][good], seg[good], w_seg[good],
    ...
    if problematic_freqs:
        logger.warning(
            f"Covariance estimation issues between {min(problematic_freqs):.2f}-"
            f"{max(problematic_freqs):.2f} Hz: Modify min_nonzero "
            f"(current: {min_nonzero}), mincoh (current: {mincoh}), maxdt "
            f"(current: {maxdt}) and/or coda_cycles (current: {coda_cycles})")

    return dvv * 100, err * 100, wf, coda_diagnostics
def compute_wct_dvv_adaptive2(freqs, tvec, WXamp, Wcoh, delta_t, distance_km=None, 
                           coda_detector=None, ref_waveform=None, 
                           coda_cycles=20, mincoh=0.5, maxdt=0.2, 
                           min_nonzero=0.25, freqmin=0.1, freqmax=2.0):
    """
...
def _peak_normalise(x):
    """Normalise by peak absolute amplitude. Returns None if not normalisable."""
    peak = np.nanmax(np.abs(x))
    if not np.isfinite(peak) or peak == 0:
        return None
    return x / peak
    
def _prepare_waveform(x, norm, label, ctx):
    """Reject non-finite or zero-amplitude traces before they reach pycwt.

    _peak_normalise uses nanmax, so it will happily return a trace that
    still contains NaN. A single NaN entering wavelet.cwt propagates
    through the FFT and makes the entire WCT output NaN, silently.
    """
    x = np.asarray(x, dtype=float)
    if not np.isfinite(x).all():
        n_bad = int((~np.isfinite(x)).sum())
        logger.warning(f"{label}: {n_bad}/{x.size} non-finite samples | {ctx}")
        return None
    if not norm:
        return x
    out = _peak_normalise(x)
    if out is None:
        logger.warning(f"{label}: zero or non-finite peak amplitude | {ctx}")
    return out
        ...
        for component in components_to_compute:
            ...
            if ref_type == 'static':
                try:
                    ...
                    static_waveform = _prepare_waveform(
                        ref, wct_norm, "Static reference",
                        f"{pair} {component} f{filterid} @ {date.date()}")
                    if static_waveform is None:
                        continue
...
#                    stack_td = pd.Timedelta(mov_stack[0])
#                    step_td  = pd.Timedelta(mov_stack[1])
#                    if stack_td <= pd.Timedelta(0) or step_td <= pd.Timedelta(0):
#                        logger.error(f"Invalid mov_stack {mov_stack}, skipping")
#                        continue
#                    stack_size = stack_td / pd.Timedelta(days=1)
#                    step_days  = step_td  / pd.Timedelta(days=1)
                    
                    ctx = f"{pair} {component} f{filterid} m{mov_stack} @ {date.date()}"
                    ...
                    new_waveform = _prepare_waveform(
                        waveform, wct_norm, "Current waveform", ctx)
                    if new_waveform is None:
                        continue
                    #waveform     = day_data.iloc[0].values
                    #new_waveform = waveform / waveform.max() if wct_norm else waveform
#                    new_waveform = _peak_normalise(waveform) if wct_norm else waveform
#                    if new_waveform is None:
#                        logger.warning(f"Current waveform has zero/NaN amplitude | {pair} ... @ {date.date()}")
#                        continue
                        ...
                        if ref_end_param == 'auto':
                            ...
                            if ref_end >= current_stack_start:
                                logger.error(
                                    ...
                                    f"{pair} {component} f{filterid} m{mov_stack} @ {date.date()}, skipping")
                                continue 
                        ref_lag_days = (date - ref_end).days 
                    
                        # --- resolve ref_start and window width (fixed, never changes) ---
                        ref_window_days = stack_size if ref_start_param == 'auto' else abs(int(ref_start_param))
                        ref_start = ref_end - pd.Timedelta(days=ref_window_days)
#                        if ref_start_param == 'auto':
#                            ref_start       = ref_end - pd.Timedelta(days=stack_size)
#                            ref_window_days = stack_size
#                        else:
#                            ref_start       = ref_end - pd.Timedelta(days=abs(int(ref_start_param)))
#                            ref_window_days = abs(int(ref_start_param))

                        logger.info(f"Reference window: ({ref_start.date()}, {ref_end.date()}] "
                                    f"= {ref_window_days}d ending {ref_lag_days}d before {date.date()}")
                                    
                        # --- search in [ref_start, ref_end] ---
                        # no shifting, no contiguity check: gap handling is integrator's responsibility
                        ...
                        if window_data.empty:
                            logger.warning(
                                f"No reference stack in [{ref_start.date()}, {ref_end.date()}] for "
                                f"{station1}:{station2} {component} f{filterid} "
                                f"m{mov_stack} @ {date.date()}, skipping")
                            continue
                    
                        # average all stacks in window (single stack for auto/auto and int(M)/auto)
                        #ref_values   = window_data.values.mean(axis=0)
                        ref_block   = window_data.values
                        finite_rows = np.isfinite(ref_block).all(axis=1)
                        peaks       = np.full(ref_block.shape[0], np.nan)
                        if finite_rows.any():
                            peaks[finite_rows] = np.max(np.abs(ref_block[finite_rows]), axis=1)
                        valid_rows  = finite_rows & (peaks > 0)

                        n_ref_rejected = int((~valid_rows).sum())
                        if n_ref_rejected:
                            logger.info(f"Rejected {n_ref_rejected}/{len(valid_rows)} reference stacks "
                                        f"(non-finite or zero amplitude) | {ctx}")
                        if valid_rows.sum() == 0:
                            logger.warning(f"No usable reference stack in ({ref_start.date()}, {ref_end.date()}] "
                                           f"| {ctx}")
                            continue

                        rows = ref_block[valid_rows]
                        if wct_norm:
                            rows = rows / peaks[valid_rows][:, None]   # per-stack normalisation first
                        ref_values   = rows.mean(axis=0)   # every row is finite now
                        n_ref_stacks = int(valid_rows.sum())
                        used_idx     = window_data.index[valid_rows]

                        n_ref_expected = max(1, int(round(ref_window_days / step_days)))
                        if n_ref_stacks < n_ref_expected:
                            logger.warning(f"Reference built from {n_ref_stacks}/{n_ref_expected} "
                                           f"expected stacks | {ctx}")
                        
   
                        #ori_waveform = ref_values / ref_values.max() if wct_norm else ref_values
                        ori_waveform = _prepare_waveform(
                            ref_values, wct_norm, "Averaged moving reference", ctx)
                        if ori_waveform is None:
                            continue
                        
                        # separation = lag from last stack in ref window to current date
                        # recorded honestly so integrator can detect gaps
                        #separation = (date - window_data.index[-1]).days
                        ref_last  = used_idx.max()
                        ref_first = used_idx.min()          # don't assume sorted
                        separation        = (date - ref_last).days
                        ref_centroid      = used_idx.to_series().mean()
                        ref_effective_lag = (date - ref_centroid).days       # lag of the averaged reference
                        ref_data_span_days = (ref_last - ref_first).days + stack_size    # true span of underlying data
                        expected_sep = ref_lag_days + step_days-1
                        if separation > expected_sep:
                            logger.warning(
                                f"Large separation={separation}d > expected={expected_sep}d | "
                                f"ref_end={ref_last.date()}, current={date.date()}, "
                                f"{pair} {component} f{filterid} m{mov_stack} — "
                                f"gap will be handled during integration")

                    else:
                        # static reference loaded before the mov_stack loop
                        ori_waveform = static_waveform
                        separation         = np.nan
                        ref_lag_days       = np.nan   # a static reference has no meaningful lag
                        ref_window_days    = np.nan
                        ref_effective_lag  = np.nan
                        ref_data_span_days = np.nan
                        n_ref_stacks       = 1
                        n_ref_rejected     = 0      # add this line
                        n_ref_expected     = 1      # add this line
                    ...
                    save_day_wct_results_adaptive(
                        ...
                        curr_window_days, ref_window_days, step_days, ref_lag_days, separation,
                        ref_effective_lag, ref_data_span_days, n_ref_stacks, n_ref_expected, n_ref_rejected
                    )
...
#        logger.warning(f"Job partially failed: {pair} - {day}. "
#                       f"{operations_succeeded} succeeded, {operations_failed} failed.")
#        return False
...
def save_day_wct_results_adaptive(station1, station2, date, component, filterid, mov_stack, 
                                 taxis, dvv, err, coh, freqs, inx, coda_diagnostics, curr_window_days, ref_window_days, step_days, ref_lag_days, separation, ref_effective_lag=np.nan, ref_data_span_days=np.nan,
                                  n_ref_stacks=np.nan, n_ref_expected=np.nan, n_ref_rejected=np.nan):
    """
    ...
    def _num(key):
        v = coda_diagnostics.get(key)
        return np.nan if v is None else float(v)
    coda_distance_km       = _num('distance_km')
    coda_surface_wave_time = _num('surface_wave_time')
    coda_start             = _num('coda_start')
    coda_constraint        = coda_diagnostics.get('constraint') or 'unknown'
    ...
    np.savez_compressed(
        ...
        coda_distance_km=coda_distance_km,
        coda_surface_wave_time=coda_surface_wave_time,
        coda_start=coda_start,
        coda_constraint=coda_constraint,
        curr_window_days=curr_window_days,
        ref_window_days=ref_window_days,
        step_days=step_days,
        ref_lag_days=ref_lag_days,
        actual_separation=separation,
        ref_effective_lag=ref_effective_lag,
        ref_data_span_days=ref_data_span_days,
        n_ref_stacks=n_ref_stacks,
        n_ref_expected=n_ref_expected,
        n_ref_rejected=n_ref_rejected,
    )
...
def xwt(trace_ref, trace_current, fs, ns=3, nt=0.25, vpo=12, freqmin=0.1, freqmax=8.0, nptsfreq=100, wavelet_type=('Morlet',6.), maxdt=0.1):
    ...
    WXspec = np.full(crossCFS.shape, np.nan, dtype=complex)
    Wcoh   = np.full(crossCFS.shape, np.nan, dtype=float)

    WXspec[valid_mask] = crossCFS[valid_mask] / (np.sqrt(cfs1[valid_mask]) * np.sqrt(cfs2[valid_mask]))
    Wcoh[valid_mask]   = np.abs(crossCFS[valid_mask]) ** 2 / (cfs1[valid_mask] * cfs2[valid_mask])
...
def _cfg(name, default, cast=str):
    v = get_config(db, name)
    if v is None or v == '':
        logger.warning(f"Config '{name}' missing or empty, using default {default!r}")
        return default
    try:
        return cast(v)
    except (TypeError, ValueError):
        logger.warning(f"Config '{name}'={v!r} not parseable as {cast.__name__}, "
...
def main(loglevel="INFO", batch_size=5):
    ...
    try:
        ...
        if params.ref_type == 'moving' and params.wct_ref_end != 'auto':
            try:
                _re = int(params.wct_ref_end)
            except (TypeError, ValueError):
                logger.error(f"wct ref_end must be 'auto' or an integer number of days, "
                             f"got {params.wct_ref_end!r}")
                return
            if _re >= 0:
                logger.error(f"ref_end must be negative (days before current date), got {_re}")
                return
            for ms in params.mov_stack:
                _S = int(''.join(filter(str.isdigit, str(ms[0]).split('d')[0])))
                if -_re < _S:
                    logger.error(f"ref_end={_re}d overlaps the current stack for mov_stack={ms} "
                                 f"(stack_size={_S}d); this mov_stack will produce no output")

        if params.ref_type == 'moving' and params.wct_ref_start != 'auto':
            try:
                int(params.wct_ref_start)
            except (TypeError, ValueError):
                logger.error(f"wct ref_begin must be 'auto' or an integer, got {params.wct_ref_start!r}")
                return
