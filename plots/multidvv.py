...
###            
            ...
            tmps2year = list(set(list(tmp2.index.year)))
            tmps2year = tmps2year[-1:] + tmps2year[:-1]

            for i in tmps2year:
                oneyear[str(i)] = tmp2.loc[str(i)]
                oneyear[str(i)].index = oneyear[str(i)].index.strftime('%m-%d')
                            
            allyear = pd.DataFrame.from_dict(oneyear) 
            
            plt.plot(allyear.index, allyear, label=allyear.columns.tolist())
            
            allyear.to_csv('/home/ulb/gtime/lbrenot/MSNoise_dir/allyear'+str(mov_stack)+'_f'+str(filterid)+'.csv')
