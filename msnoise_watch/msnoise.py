...
@cli.command(name='compute_zoom_mwcs')
@click.pass_context
def compute_zoom_mwcs(ctx):
    """Computes the MWCS jobs"""
    from ..msnoise_mutations import zoom_mwcs
    threads = ctx.obj['MSNOISE_threads']
    delay = ctx.obj['MSNOISE_threadsdelay']
    loglevel = ctx.obj['MSNOISE_verbosity']
    if threads == 1:
        zoom_mwcs(loglevel=loglevel)
    else:
        from multiprocessing import Process
        processes = []
        for i in range(threads):
            p = Process(target=zoom_mwcs, kwargs={"loglevel": loglevel})
            p.start()
            processes.append(p)
            time.sleep(delay)
        for p in processes:
...
@cli.command(name='compute_zoom_dtt')
@click.pass_context
@click.option('-i', '--interval', default=1.0, help='Number of days before now to\
 search for modified Jobs')
def compute_zoom_dtt(ctx, interval):
    """Computes the dt/t jobs based on the new MWCS data"""
    from ..msnoise_mutations import zoom_dtt
    threads = ctx.obj['MSNOISE_threads']
    delay = ctx.obj['MSNOISE_threadsdelay']
    loglevel = ctx.obj['MSNOISE_verbosity']
    if threads == 1:
        zoom_dtt(loglevel=loglevel)
    else:
        from multiprocessing import Process
        processes = []
        for i in range(threads):
            p = Process(target=zoom_dtt, kwargs={"loglevel": loglevel})
            p.start()
            processes.append(p)
            time.sleep(delay)
        for p in processes:
...
@click.option('-o', '--outfile', help='Output filename (?=auto)',
              default=None, type=str)
@click.pass_context
def zoom_dvv(ctx, mov_stack, comp, dttname, filterid, pair, all, show, outfile):
    """Plots the dv/v (parses the dt/t results)\n
    Individual pairs can be plotted extra using the -p flag one or more times.\n
    Example: msnoise plot dvv -p ID_KWUI_ID_POSI\n
    Example: msnoise plot dvv -p ID_KWUI_ID_POSI -p ID_KWUI_ID_TRWI\n
    ...
    """
    if ctx.obj['MSNOISE_custom']:
        from dvv import zoomerrdvv
    else:
        from ..msnoise_mutations import zoomerrdvv
    zoomerrdvv(mov_stack, dttname, comp, filterid, pair, all, show, outfile)
