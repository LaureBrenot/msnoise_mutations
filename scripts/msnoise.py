...
@click.option('-o', '--outfile', help='Output filename (?=auto)',
              default=None, type=str)
@click.pass_context
def errdvv(ctx, mov_stack, comp, dttname, filterid, pair, all, show, outfile):
    """Plots the dv/v (parses the dt/t results)\n
    Individual pairs can be plotted extra using the -p flag one or more times.\n
    Example: msnoise plot dvv -p ID_KWUI_ID_POSI\n
    Example: msnoise plot dvv -p ID_KWUI_ID_POSI -p ID_KWUI_ID_TRWI\n
    ...
    """
    if ctx.obj['MSNOISE_custom']:
        from errdvv import main
    else:
        from ..plots.errdvv import main
    main(mov_stack, dttname, comp, filterid, pair, all, show, outfile)
...
@click.option('-o', '--outfile', help='Output filename (?=auto)',
              default=None, type=str)
@click.pass_context
def multidvv(ctx, mov_stack, comp, dttname, filterid, pair, all, show, outfile):
    """Plots the dv/v (parses the dt/t results)\n
    Individual pairs can be plotted extra using the -p flag one or more times.\n
    Example: msnoise plot dvv -p ID_KWUI_ID_POSI\n
    Example: msnoise plot dvv -p ID_KWUI_ID_POSI -p ID_KWUI_ID_TRWI\n
    ...
    """
    if ctx.obj['MSNOISE_custom']:
        from multidvv import main
    else:
        from ..plots.multidvv import main
    main(mov_stack, dttname, comp, filterid, pair, all, show, outfile)



@cli.command(name = 'snr_score', context_settings=dict(ignore_unknown_options=True,))
@click.option('-f', '--filterid', default=1, help='Filter ID')
@click.option('-c', '--comp', default="ZZ", help='Components (ZZ, ZR,...)')
@click.option('-m', '--mov_stack', default=1,
              help='Mov Stack to read from disk')
@click.option('-over', '--overwrite', help='Overwrite the scores in the file?',
...
@click.pass_context
def snr_score(ctx, filterid, comp, mov_stack, overwrite, show, outfile,
                  refilter, extra_args):
    """Compute snr scores between sta1 and sta2 (parses the CCFs)\n
    All pairs"""


    if ctx.obj['MSNOISE_custom']:
        from snr_score import main
    else:
        from ..plots.snr_score import main
    main(filterid, comp, mov_stack, show, outfile, refilter,
         **extra_args)



@cli.command(name='zoom_compute_mwcs')
@click.pass_context
def zoom_dvv_compute_mwcs(ctx):
    """Computes the MWCS jobs"""
    from ..zoom_s05compute_mwcs import main
    threads = ctx.obj['MSNOISE_threads']
    delay = ctx.obj['MSNOISE_threadsdelay']
    loglevel = ctx.obj['MSNOISE_verbosity']
    if threads == 1:
...
@cli.command(name='zoom_compute_dtt')
@click.pass_context
@click.option('-i', '--interval', default=1.0, help='Number of days before now to\
 search for modified Jobs')
def zoom_dvv_compute_dtt(ctx, interval):
    """Computes the dt/t jobs based on the new MWCS data"""
    from ..zoom_s06compute_dtt import main
    threads = ctx.obj['MSNOISE_threads']
    delay = ctx.obj['MSNOISE_threadsdelay']
    loglevel = ctx.obj['MSNOISE_verbosity']
    if threads == 1:
...
@click.option('-o', '--outfile', help='Output filename (?=auto)',
              default=None, type=str)
@click.pass_context
def zoomerrdvv(ctx, mov_stack, comp, dttname, filterid, pair, all, show, outfile):
    """Plots the dv/v (parses the dt/t results)\n
    Individual pairs can be plotted extra using the -p flag one or more times.\n
    Example: msnoise plot dvv -p ID_KWUI_ID_POSI\n
    Example: msnoise plot dvv -p ID_KWUI_ID_POSI -p ID_KWUI_ID_TRWI\n
    ...
    """
    if ctx.obj['MSNOISE_custom']:
        from zoomerrdvv import main
    else:
        from ..plots.zoomerrdvv import main
    main(mov_stack, dttname, comp, filterid, pair, all, show, outfile)
